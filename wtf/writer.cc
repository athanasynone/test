...
void VisualizationWriter::writeMajor(EL::PathSolution *solution)
{
    ...
    for( int i=0; i < solution->numPaths(); i++ )
    {
        ...
        if( interesting )
        {
            for( int j=1; j < path.m_points.size()-1; j++ )
            {
                const EL::Vector3& p0 = path.m_points[j];
                sprintf(m_writeBuf, "%f", p0[0]);
                m_socket->write(strlen(m_writeBuf), m_writeBuf);
                sprintf(m_writeBuf, "%f", p0[1]);
                m_socket->write(strlen(m_writeBuf), m_writeBuf);
                sprintf(m_writeBuf, "%f", p0[2]);
                m_socket->write(strlen(m_writeBuf), m_writeBuf);
            }
            sprintf(m_writeBuf, "new source");
            m_socket->write(strlen(m_writeBuf), m_writeBuf);
        }
	    if( i == solution->numPaths() - 1 )
	    {
		    sprintf(m_writeBuf, "all done");
            m_socket->write(strlen(m_writeBuf), m_writeBuf);
	    }
    }
